...
for i in range(dat_files):
    ...
    f = bz2.BZ2File("C:\Users\Anai\Desktop\lagodata\Eventos"+str(i+1)+".dat.bz2")  #leer el archivo comprimido
    print("ya leyo "+str(i))
    ...
    with open("Crossing.txt", 'a') as result:
        for item in crossing_muons:
            result.write("%s\n" % int(item))      
    
    with open("Decaying.txt", 'a') as result:
        for item in decaying_muons:
            result.write("%s\n" % int(item)) 
    
    with open("Michel.txt", 'a') as result:
